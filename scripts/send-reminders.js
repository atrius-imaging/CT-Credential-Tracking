...
function fetchJSON(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { "User-Agent": "CT-Credentials/1.0" } }, res => {
      let data = "";
...
const { techs: TECHS } = await fetchJSON("https://raw.githubusercontent.com/atrius-imaging/CT-Credential-Tracking/main/data.json?t=" + Date.now());
console.log("Loaded", TECHS.length, "techs from data.json");
...
console.log("User length:", GMAIL_USER.length, "| Password length:", GMAIL_PASSWORD.trim().length);
const transport=nodemailer.createTransport({service:"gmail",auth:{user:GMAIL_USER.trim(),pass:GMAIL_PASSWORD.trim()}});
